...
asteroid.prototype = new worldObject;
function asteroid(parent, normalDirection) {
...
asteroid.prototype.initBuffers = function (normalDirection) {
    vertices = [];
    textureCoords = [];
    normalData = [];
    var nbVertice = 0;
    ...
    for (var lat = -90; lat <= phiMax; lat += pasLat) {
        for (var longi = 0; longi <= tetaMax; longi += pasLong) {
            rand1 = Math.random();
            rand2 = Math.random();
            vertices = vertices.concat(pol2Cart(longi*rand1, lat*rand2, normalDirection));
            normalData = normalData.concat(pol2Cart(longi*rand1, lat*rand2, normalDirection));
            textureCoords = textureCoords.concat([longi*rand1 / tetaMax, (90 + lat*rand2) / (90 + phiMax)]);
            if (longi != tetaMax) {
                if (lat < phiMax) {
